...
def patch_post(request, id):
    try:
        ...
        if post_validate_slug.exists() and post != post_validate_slug.first():
            return Response({"message": "Nombre de publicación ya esta en uso"}, status=status.HTTP_400_BAD_REQUEST)
...
def delete_image_post(request, post_id):
    ...
    try:
        ...
        try:
            ...
            post.save()
            return Response({"message": "Imagen de portada eliminada correctamente."}, status=status.HTTP_200_OK)
        except:
            return Response({"message": "No se puedo eliminadar."}, status=status.HTTP_400_BAD_REQUEST)
    except Exception as e:
        return Response(e, status=status.HTTP_404_NOT_FOUND)
...
def reaction_list(request, post_id):
    ...
    top_reactions = sorted(counted_reactions, key=lambda x: x['count'], reverse=True)[:3]
    ...
    # Crear la respuesta
    response_data = {
        'reactions': [{'emoji': item['emoji'], 'count': item['count']} for item in top_reactions],
        'count': total_count,

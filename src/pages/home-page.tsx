...
import { allPosts, allTags } from "@/services/blog";
...
const HomePage = () => {
  ...
  // allTags useQuery
  const { data: tags } = useQuery(["allTags"], allTags);

  return (
    <>
      <Head title="Inicio" />
      <section className="max-w-7xl mx-auto my-16">
        <h1 className="block text-3xl font-bold sm:text-4xl lg:text-6xl">
          Stories, & trends shaping the future of business.{" "}
          <span className="text-blue-600">Insights</span>
        </h1>
        <div className="grid grid-cols-2 divide-y divide-border divide-x max-w-xl py-10">
          <div className="flex items-center gap-2 px-3 py-2">
            <span>Grow your Business</span>
            <ArrowRightCircleIcon />
          </div>
          <div className="flex items-center gap-2 px-3 py-2">
            <span>Finance your Business</span>
            <ArrowRightCircleIcon />
          </div>
          <div className="flex items-center gap-2 px-3 py-2">
        ...
        <div className="grid grid-cols-5 gap-10">
          <div className="col-span-2">
            <h3 className="font-bold text-2xl">Ultima publicación</h3>
            <div className="flex flex-wrap mt-2 gap-x-2 gap-y-2">
              {tags?.map((t) => (
                <Link
                  className="px-2 py-1 font-medium hover:bg-muted border border-border text-sm text-muted-foreground rounded-2xl"
                  key={t.id}
                  to={`/t/${t.name}`}
                >
                  {t.name}
          ...
          <div className="col-span-3">
            <article className="relative group border border-border p-10 rounded-2xl">
              <img
        ...
        <h2 className="my-10 font-bold text-3xl">Más reciente</h2>
      ...
      <main className="mx-auto max-w-[85rem] px-4 my-8 sm:px-6 lg:px-8 grid gap-16 grid-cols-3">
        <section className="col-span-2 divide-y divide-muted">
          {posts && posts?.length
            ? posts.map((p) => (
                <article key={p.id} className="py-3">
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    {p.author?.picture && (
                      <Link to={`/@/${p.author.username}`}>
                        <img
                          src={p.author?.picture as string}
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span>&bull;</span>
                      <time>{fromNow(p.created_at || new Date())}</time>
                    </div>
                  </div>
                  <Link to={`/post/${p.slug}`}>
                    <div className="flex items-center justify-between">
                      <div className="my-3">
                        ...
                        <p className="text-sm">
                          New Zeland's Dark Sky Project at Lake Tekapo
                        </p>
                      </div>
                      {p.cover_image && (
                        <img
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex gap-2">
                      {p.tags?.map((t) => (
                        <Link
                          className="bg-muted text-muted-foreground font-medium rounded-lg text-sm px-1 py-0.5"
                          key={t.id}
                          to={`/t/${t.name}`}
                        >
                          {t.name}
        ...
        <aside className="col-span-1">
          ...
          <div className="flex flex-wrap mt-2 gap-x-2 gap-y-2">
            {tags?.map((t) => (
              <Link
                className="px-2 py-1 font-medium bg-muted text-muted-foreground rounded-2xl"
                key={t.id}
                to={`/t/${t.name}`}
              >
                {t.name}
      ...
      <div className="mx-auto max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <div className="flex flex-col h-full border shadow-sm group rounded-xl border-border bg-card">
            ...
            <div className="flex mt-auto border-t divide-x border-border divide-border dark:divide-muted dark:border-muted">
              <a
                className="inline-flex items-center justify-center w-full gap-2 px-4 py-3 text-sm font-medium align-middle transition-all shadow-sm rounded-bl-xl hover:bg-muted focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 focus:ring-offset-white dark:border-muted dark:bg-dark dark:hover:bg-dark dark:hover:text-light dark:focus:ring-offset-dark sm:p-4"
                href="#"
              >
                View sample
              </a>
              <a
                className="inline-flex items-center justify-center w-full gap-2 px-4 py-3 text-sm font-medium align-middle transition-all shadow-sm rounded-br-xl hover:bg-muted focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 focus:ring-offset-white dark:border-muted dark:bg-dark dark:hover:bg-dark dark:hover:text-light dark:focus:ring-offset-dark sm:p-4"
                href="#"
              >
                View API
              </a>
      ...
      <div className="max-w-5xl px-4 py-10 mx-auto sm:px-6 lg:px-8 lg:py-14">
        <div className="grid gap-4 sm:grid-cols-2 sm:gap-6 lg:grid-cols-3">
          <a
            className="flex flex-col transition border shadow-sm border-border group rounded-xl hover:shadow-md dark:border-muted dark:bg-dark bg-card"
            href="#"
          >
            <div className="p-4 md:p-5">
              <div className="flex">
                ...
                <div className="ml-5 grow">
                  ...
                  <p className="text-sm">Get help from 40k+ Preline users</p>
                </div>
              </div>
            </div>
          </a>
            ...
            <div className="p-4 md:p-5">
              <div className="flex">
                ...
                <div className="ml-5 grow">
                  ...
                  <p className="text-sm">Just head to «Help» in the app</p>
                </div>
              </div>
            </div>
          </a>
          ...
          <a
            className="flex flex-col transition border shadow-sm group border-border rounded-xl hover:shadow-md dark:border-muted dark:bg-dark bg-card"
            href="#"
          >
            <div className="p-4 md:p-5">
              <div className="flex">

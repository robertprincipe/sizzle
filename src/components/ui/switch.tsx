...
>(({ className, ...props }, ref) => (
  ...
  >
    <SwitchPrimitives.Thumb
      className={cn(
        "pointer-events-none flex justify-center items-center h-5 w-5 rounded-full bg-white shadow-lg ring-0 transition-transform data-[state=unchecked]:translate-x-0 data-[state=checked]:translate-x-5"
      )}
    />
  </SwitchPrimitives.Root>
));
...
/*
{checked ? (
        <Icons.moon className="w-4 h-4 text-gray-700" />
      ) : (
        <Icons.sun className="w-4 h-4 text-gray-700" />
      )} */

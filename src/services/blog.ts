...
import { nullishImage } from "@/lib/nullish";
...
export const postDetail = (slug: string) => API.get<IPost>(`/posts/${slug}`).then(res => res.data)

export const postData = (id: string) => API.get<IPost>(`/posts/${id}/edit`).then(res => res.data)
...
export const reactionList = (post_id: string) =>
    API.get<IReactionList>(`/reaction-list/post/${post_id}`).then(res => res.data)

export const userReaction = (post_id: string) =>
    API.get<{ user_reaction: string, count: number }>(`/reaction/user/${post_id}`).then(res => res.data)

export const searchTags = (query: string) =>
    API.get<ITag[]>(`/tags/search?query=${query}`).then(res => res.data)
